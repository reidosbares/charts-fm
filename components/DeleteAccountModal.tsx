  ...
  return (
    <>
      ...
      <div className="fixed inset-0 flex items-center justify-center z-50 p-4 md:p-6">
        <div className="bg-[var(--surface-elevated)] rounded-lg md:rounded-xl shadow-xl p-4 md:p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
          ...
          {error && (
            <div className="mb-4 md:mb-6 p-3 md:p-4 bg-red-50 dark:bg-red-950/40 border border-red-400 dark:border-red-800 text-red-700 dark:text-red-400 rounded text-xs md:text-sm">
              {error}
            </div>
          )}

          <div className="mb-6 md:mb-8">
            <p className="text-sm md:text-base text-[var(--text-secondary)] mb-4 md:mb-6 font-semibold">
              {t('warning')}
            </p>

            {isLoadingGroups ? (
              <p className="text-xs md:text-sm text-[var(--text-secondary)] mb-4">{t('loadingGroups')}</p>
            ) : ownedGroups.length > 0 ? (
              <div className="mb-4 md:mb-6">
                <p className="text-xs md:text-sm text-[var(--text-secondary)] mb-2 font-medium">
                  {t('ownedGroupsMessage')}
                </p>
                <ul className="text-xs md:text-sm text-[var(--text-secondary)] list-disc list-inside mb-4 space-y-1 bg-[var(--surface-base)] p-3 md:p-4 rounded">
            ...
            <div className="mb-4 md:mb-6">
              ...
              <ul className="text-xs md:text-sm text-[var(--text-secondary)] list-disc list-inside mb-4 space-y-1">
            ...
            <div>
              <label htmlFor="confirmText" className="block text-xs md:text-sm font-medium text-[var(--text-secondary)] mb-2">
              ...
              <input
                ...
                className="w-full px-3 md:px-4 py-2.5 md:py-2 text-sm md:text-base border border-[var(--border-strong)] rounded-lg bg-[var(--surface-card)] text-[var(--text-primary)] focus:ring-2 focus:ring-red-500 focus:border-red-500"
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-end">
            <button
              onClick={onClose}
              disabled={isLoading}
              className="px-4 py-2.5 md:py-2 bg-[var(--surface-base)] text-[var(--text-primary)] rounded-lg hover:bg-[var(--surface-base)] transition-colors disabled:opacity-50 disabled:cursor-not-allowed text-sm md:text-base font-medium min-h-[44px] sm:min-h-0 order-2 sm:order-1 border border-[var(--border-subtle)]"
            >
              {t('cancel')}
            </button>
            <button
