...
function QuickStats({ totalVS, totalPlays, majorDriver, chartMode, numberOneTracks, numberOneAlbums, weeksAtNumberOne }: QuickStatsProps) {
  ...
  return (
    <div className="bg-white/40 dark:bg-[rgb(var(--surface-card-rgb)/0.4)] backdrop-blur-md rounded-xl p-4 md:p-6 border border-white/30 dark:border-white/10" style={{ contain: 'layout style paint' }}>
      <h2 className="text-lg md:text-xl font-bold text-[var(--text-primary)] mb-3 md:mb-4">{t('title')}</h2>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-4">
        {showVS && totalVS !== null && (
          <div>
            <div className="text-xs md:text-sm text-[var(--text-secondary)] mb-1">{t('totalVS')}</div>
            <div className="text-xl md:text-2xl font-bold text-[var(--text-primary)]">{totalVS.toFixed(2)}</div>
          </div>
        )}
        <div>
          <div className="text-xs md:text-sm text-[var(--text-secondary)] mb-1">{t('totalPlays')}</div>
          <div className="text-xl md:text-2xl font-bold text-[var(--text-primary)]">{totalPlays.toLocaleString()}</div>
        </div>
        {majorDriver && (
          <div>
            <div className="text-xs md:text-sm text-[var(--text-secondary)] mb-1">{t('majorChartDriver')}</div>
            <Link
        ...
        {weeksAtNumberOne !== undefined && (
          <div>
            <div className="text-xs md:text-sm text-[var(--text-secondary)] mb-1">{t('weeksAtNumberOne')}</div>
            <div className="text-xl md:text-2xl font-bold text-[var(--text-primary)]">{weeksAtNumberOne}</div>
          </div>
        )}
        {showNumberOnes && numberOneTracks !== undefined && (
          <div>
            <div className="text-xs md:text-sm text-[var(--text-secondary)] mb-1">{t('numberOneTracks')}</div>
            <div className="text-xl md:text-2xl font-bold text-[var(--text-primary)]">{numberOneTracks}</div>
          </div>
        )}
        {showNumberOnes && numberOneAlbums !== undefined && (
          <div>
            <div className="text-xs md:text-sm text-[var(--text-secondary)] mb-1">{t('numberOneAlbums')}</div>
            <div className="text-xl md:text-2xl font-bold text-[var(--text-primary)]">{numberOneAlbums}</div>
          </div>
        )}
      </div>
    </div>
