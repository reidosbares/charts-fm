...
async function fetchEntryImage(entry: { chartType: string; name: string; artist?: string }): Promise<string | null> {
  try {
    if (entry.chartType === 'artists') {
    ...
    if (entry.chartType === 'albums' && entry.artist) {
      const res = await fetch(`/api/images/album?artist=${encodeURIComponent(entry.artist)}&album=${encodeURIComponent(entry.name)}`)
      const json = await res.json()
      return json.imageUrl ?? null
    }
    if ((entry.chartType === 'tracks' || entry.chartType === 'albums') && entry.artist) {
...
export default function GroupTrendsTab({ groupId }: GroupTrendsTabProps) {
  ...
  useEffect(() => {
    if (!data?.trends) {
      setBiggestClimberImage(null)
      setNewEntriesImages([])
      return
    }
    const climbers = (data.trends.biggestClimbers as any[]) || []
    const entries = (data.trends.newEntries as any[]) || []
    setBiggestClimberImage(null)
    setNewEntriesImages([])
    if (climbers.length > 0) {
      fetchEntryImage(climbers[0]).then(setBiggestClimberImage)
    }
    if (entries.length > 0) {
      Promise.all(entries.slice(0, 3).map((e: any) => fetchEntryImage(e))).then(setNewEntriesImages)
    }
  }, [data])
  ...
  return (
    <div>
      ...
      {/* Bento grid: Row 2+ - MVP featured, then climber | new entries, diverse | fun facts */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className={cardBase}>
            ...
            <div className="space-y-2 md:space-y-3">
              {newEntries.slice(0, 3).map((entry: any, idx: number) => {
                ...
                const entryImage = newEntriesImages[idx] ?? null
                return (
                  <div
                    key={idx}
                  ...
                  >
                    ...
                    {entryImage ? (
                      <div className="w-10 h-10 md:w-12 md:h-12 rounded-lg overflow-hidden flex-shrink-0 bg-[var(--theme-primary-lighter)] ring-1 ring-[var(--theme-border)]">
                        <SafeImage src={entryImage} alt={entry.name} className="object-cover w-full h-full" />
                      </div>
                    ) : (
                      <FontAwesomeIcon
        ...
        {/* Fun Facts - full width on desktop */}
        {funFacts.length > 0 && (
