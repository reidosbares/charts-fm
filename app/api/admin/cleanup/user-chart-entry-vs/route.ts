...
export async function POST() {
  try {
    ...
    // Delete UserChartEntryVS entries older than 10 weeks
    const result = await prisma.userChartEntryVS.deleteMany({
      where: {
        weekStart: {
          lt: cutoffDate,
        },
    ...
    console.error('UserChartEntryVS cleanup error:', error)
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error
            ? error.message
            : 'Failed to cleanup UserChartEntryVS entries',
      },
      { status: 500 }
    )
  }
